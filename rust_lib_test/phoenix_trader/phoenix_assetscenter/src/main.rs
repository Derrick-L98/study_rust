...
use crate::{config::settings::Settings, protofiles::assetscenter::phoenixassetscenter_server::PhoenixassetscenterServer};
use anyhow::Result;
use server::ServerHandler;
use utility::{loggings, timeutil};
...
async fn server_run(mut svr: ServerHandler, settings: &Settings) -> Result<(), Box<dyn std::error::Error>> {
  ...
  tonic::transport::Server::builder()
    .add_service(PhoenixassetscenterServer::new(svr))
