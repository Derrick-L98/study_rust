...
public:
    ...
    void receiveNamingInfo(const ServiceInfo &serviceInfo){
        ...
        ServiceInfo test = serviceInfo;
		list <Instance> instances = test.getHosts();
		char url[32] = "";
		for (list<Instance>::iterator it = instances.begin();
			it != instances.end(); it++) {
			cout << "Instance:" << it->toString() << endl;
			sprintf(url, "%s", it->toInetAddr().c_str());
            if (url != NULL) {
                break;
            }
		}
        cout << "server ip" << url << endl;
        cout << "===================================" << endl;
...
int main() {
    ...
    props[PropertyKeyConst::UDP_RECEIVER_PORT] = "30621";
