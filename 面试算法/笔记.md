...
查询去重: distinct 
例子:select distinct university from user_profile 
...
作者：北山北_d41c
链接：https://www.jianshu.com/p/a10b8d9aaf5c
来源：简书
著作权归作者所有。商业转载请联系作者获得授权，非商业转载请注明出处。
```
...
>   linux创建文件的方法：1、使用touch方法，代码为【touch yy..】；2、使用vi和vim，代码为【vi test.txt】；3、使用【>、>>】，代码为【> test.ini】；4、使用cp创建文件；5、使用cat创建文件。

![img](https://img.php.cn/upload/article/202102/22/2021022211484595804.jpg)
...
卸载
which protoc 
rm /usr/local/bin/protoc查看版本：protoc --version
...
sudo apt install -y cmake//用于编译(关键)
...
//遇到问题
    编译过程中找不到 zconf.h、zlib.h 两个文件文件
    解决： 全局查找两个文件，然后拷贝到usr/include 下
```
...
理解起来并不难，难点在给概念的含义把握上，看一下几个核心概念，当然肯定看不懂的，看懂也没用，还是搞不清楚。可以直接跳到理解部分。
...
端点（Peer）：AMQP对话的任意一方。一个AMQP连接包括两个端点（一个是客户端，一个是服务器）。搭档（Partner）：当描述两个端点之间的交互过程时，使用术语“搭档”来表示“另一个”端点的简记法。比如我们定义端点A和端点B，当它们进行通信时，端点B是端点A的搭档，端点A是端点B的搭档。
...
```rust
命中率: 当某个请求能通过访问缓存而得到响应时,成为缓存命中.缓存命中率越高,缓存的利用率也就越高;
最大空间:缓存通常位于内存中,内存的空间通常比磁盘空间小,因此缓存的最大空间不可能非常大.当缓存存放的数据量超过最大空间时,就需要淘汰部分数据来存放新到达的数据.
淘汰策略:
	FIFO(Frist In Frist Out):先进先出策略,在实时性的场景下,需要经常访问最新的数据,就可以使用FIFO使得先进的数据被淘汰。
	LRU(Least RecenRly Used):最近最久未使用的策略，优先淘汰最久未使用的数据，也就是上次被访问时间距离现在最久的数据，该策略可以保证内存中的数据都是热点数据，也就是经常被访问的数据，从而保证缓存命中率。
...
```rust
浏览器：当HTTP响应允许进行缓存时，浏览器会将HTML，CSS，JavaScript，图片等静态资源进行缓存。
ISP：网络服务提供商（ISP）是网络访问的第一跳，通过将数据缓存在ISP中能够大大提高用户的访问速度。
反向代理：反向代理位于服务器之前，请求与响应都需要经过反向代理，通过将数据缓存在反向代理，在用户请求反向代理时就可以直接使用缓存进行响应。
本地缓存：使用Guava Cache 将数据缓存在服务器本地内存中，服务器代码可以直接读取本地内存中的缓存，速度非常快。
分布式缓存：使用Redis Memcache等分布式缓存将数据缓存在分布式缓存系统中。相对于本地缓存来说，分布式缓存单独部署，可以根据需求分配硬件资源，不仅如此，服务器集群都可以访问分布式缓存，而本地缓存需要在服务器集群之间进行同步，实现难度和性能开销非常大。
...
```C++
sudo vim /etc/apt/sources.list//更换源的文件目录
